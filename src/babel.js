...
export default function ({types: t}){
  ...
  function dumpToAST(obj){
    ...
    Object.keys(obj).map(className => {
      props = [];
      
      Object.keys(obj[className]).map(propName => {
      ...
      CallExpression(path){
        ...
        if(isStyleSheetCreateCall(callee)){
          ...
          for(var i = 0; i < args.length; i++){
            ...
            forEachStyleSheetClassName(args[i], (classNode) => {
            
              forEachClassProp(classNode.value, (propNode) => {
                ...
                possibleIds = Object.keys(store[propName]).filter(classId => store[propName][classId] === propValue);
